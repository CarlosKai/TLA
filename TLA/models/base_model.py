...
class BaseModel(nn.Module):
    def __init__(self,
                 ...
                 taskalign: bool = False,   # whether use taskalign
                 **kwargs
                 ):
        super().__init__()
    ...
    def forward(self, x, toalign=False, labels=None, taskalign=False) -> tuple:
        """
        return: [f, y, ...]
        """
        ...
        if toalign:
            ...
            y_pos0 = self.fc(f)
            pseudo_labels = nn.Softmax(dim=1)(y_pos0)
            pseudo_index = torch.argmax(pseudo_labels, dim=1)
            w_pos = self._get_toalign_weight(f, labels=pseudo_index)
            f_pos = f * w_pos
            y_pos = self.fc(f_pos)
            return f_pos, y_pos
        else:
